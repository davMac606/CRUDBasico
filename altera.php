...
<head>
    <meta charset="UTF-8">
    <title>CRUD - Controle de alunoPHP</title>
</head>
...
<?php
    ...
    if ( ($nomeFoto != "") && (!preg_match('/^image\/(jpeg|png|gif)$/', $tipoFoto)) ) { //validção tipo arquivo
     echo "<span id='error'>Isso não é uma imagem válida</span>";
        ...
        $stmt = $pdo->prepare('UPDATE alunoPHP SET nome = :novoNome, curso = :novoCurso, fotoFile = :novaFoto WHERE ra = :ra');
        ...
        //senão mantem a foto anterior, não fazendo update do campo fotoFile
        $stmt = $pdo->prepare('UPDATE alunoPHP SET nome = :novoNome, curso = :novoCurso WHERE ra = :ra');
        $stmt->bindParam(':novoNome', $novoNome);
        $stmt->bindParam(':novoCurso', $novoCurso);
        $stmt->bindParam(':ra', $ra);
    }
